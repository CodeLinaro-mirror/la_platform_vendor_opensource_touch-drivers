...
* Created: 2026-07-23
*
* Abstract: hynitron ts debug tool driver
*
* Version: V1.0
*
*******************************************************************************/
...
static ssize_t hyn_proc_tool_read(struct file *page,char __user *user_buf, size_t count, loff_t *f_pos)
{
	...
	//triger fw down loaded
	if(hyn_tool_data->fw_dump_state){
		hyn_dump_fw(hyn_tool_data,NULL,0);
		goto tool_read_end;
	}
...
static ssize_t hyn_proc_tool_write(struct file *file, const char __user *buffer,size_t count, loff_t *data)
{
	...
	if(hyn_dump_fw(hyn_tool_data,cmd,count)){
		goto ERRO_WEND;
	}

	switch(cmd[0]){
		case DUMP_FW: //GKI use dump fw
