#ifndef HYN_TPKEY_ACTS_H
#define HYN_TPKEY_ACTS_H
...
/**delay ms */
#define hyn_mdelay(ms)                   k_busy_wait(ms*1000)
#define hyn_msleep(ms)                   k_msleep(ms)   

/**function config */
#define HYN_POWER_ON_UPDATA     (1) //touch fw updata

#define MAX_POINTS_REPORT       (1) //max report point  

#define HYN_DRIVER_VERSION      "== Hynitron V2.50 20251222 =="

#define HYN_INFO(fmt, args...)   printk("[HYN]"fmt"\n", ##args)
#define HYN_INFO2(fmt, args...)  if(hyn_data->log_level > 0)printk("[HYN]"fmt"\n", ##args)
#define HYN_INFO3(fmt, args...)  if(hyn_data->log_level > 1)printk("[HYN]"fmt"\n", ##args)
#define HYN_INFO4(fmt, args...)  if(hyn_data->log_level > 2)printk("[HYN]"fmt"\n", ##args)
#define HYN_DEBUG(fmt, args...)  printk(fmt,##args);
#define HYN_ERROR(fmt, args...)  printk("[HYN][Error]%s:"fmt"\n",__func__,##args)
#define HYN_ENTER()              printk("[HYN][enter]%s\n",__func__)
...
#define FAC_CFG1 {0xC0, 0x80, 0x30, 0x20, 0x00}
#define FAC_CFG2 {0xC0, 0x80, 0x60, 0x20, 0x00}
...
#define READ_RAW_MODE       2
#define DEEPSLEEP_MODE      3

#define CUSTOM_SENSOR_NUM_TX    (8)
#define CUSTOM_SENSOR_NUM_RX    (7)
#define CUSTOM_SENSOR_ALL       ((CUSTOM_SENSOR_NUM_TX+CUSTOM_SENSOR_NUM_RX+1)/2)*2
...
struct hyn_ts_fuc{
    ...
    int (*tp_get_test_result)(u16 *cp_buf,u16 *delta_buf);
};
